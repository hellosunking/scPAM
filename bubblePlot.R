...
qqq=function (object, assay = NULL, features, cols = c("lightgrey", 
              ...
              scale.min = NA, scale.max = NA) 
{
                                                        ...
                                                        x = "id")) + geom_point(mapping = aes_string(size = "pct.exp", 
                                                                                                     color = color.by)) + scale.func(range = c(0, dot.scale), 
                                                                                                                                     limits = c(scale.min, scale.max)) + theme(axis.title.x = element_blank(), 
                                                                                                                                                                               axis.title.y = element_blank()) + guides(size = guide_legend(title = "%Expressed")) + 
    labs(y = "Features", x = ifelse(test = is.null(x = split.by), 
  ...
  if (is.null(x = split.by)) {
    plot <- plot + guides(color = guide_colorbar(title = "Log-norm Expr"))
  }
  return(plot)
}

ppp=function (object, assay = NULL, features, cols = c("lightgrey", 
                                                       "blue"), col.min = -2.5, col.max = 2.5, dot.min = 0, dot.scale = 6, 
              group.by = NULL, split.by = NULL, scale = TRUE, scale.by = "radius", 
              scale.min = NA, scale.max = NA) 
{
  ...
  plot <- ggplot(data = data.plot, mapping = aes_string(x = "features.plot", 
                                                        y = "id")) + geom_point(mapping = aes_string(size = "pct.exp", 
                                                                                                     color = color.by)) + scale.func(range = c(0, dot.scale), 
                                                                                                                                     limits = c(scale.min, scale.max)) + theme(axis.title.x = element_blank(), 
                                                                                                                                                                               axis.title.y = element_blank()) + guides(size = guide_legend(title = "%Expr")) + 
